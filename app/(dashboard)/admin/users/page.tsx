...
export default async function AdminUsers() {
  ...
  const users: UserOut[] = await res.json().catch(() => []);
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-gray-50 border-b border-gray-100 text-gray-500">
            <tr>
              ...
              <th className="px-5 py-4 font-medium">Регіон</th>
              <th className="px-5 py-4 font-medium">Посада</th>
              <th className="px-5 py-4 font-medium">Доданий</th>
            </tr>
          </thead>
            ...
            {users.map(u => {
              ...
              return (
                <tr key={u.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-5 py-4 text-gray-500">{u.region?.name ?? '—'}</td>
                  <td className="px-5 py-4 text-gray-500">{u.job_position ?? '—'}</td>
